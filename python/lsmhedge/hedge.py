...
class HedgeConfig:
    ...
    # American/Bermudan value >= intrinsic always.
    # We approximate "exercise region" when time value is near zero:
    #    option_price - payoff <= k * price_stderr
    exercise_detection: bool = True
    exercise_k_stderr: float = 2.0

    # speed: optional caching of pricing calls using rounded state (approximate!)
...
def _cache_key(S: float, sigma: float, r: float, T: float, cfg: HedgeConfig) -> Tuple[float, float, float, float]:
    return (
...
def run_single_trade_delta_hedge(
    ...
    Book accounting:
      book_value = cash + stock*S - option_price   (while option alive)
      cash accrues at r (continuous comp, daily dt=1/252)
      transaction cost applied on every trade in shares.

    At t=0:
      receive premium, then hedge into delta shares.
    """
    ...
    # optional approximate cache
    cache: Dict[Tuple[float, float, float, float], Tuple[float, float, float, float]] = {}
    ...
    for i in range(n):
        ...
        if option_alive:
            if T_rem <= 0.0:
                ...
                # model quote
                if cfg.use_cache:
                    key = _cache_key(S, sigma, r, T_rem, cfg)
                    if key in cache:
                        option_price, delta, price_se, delta_se = cache[key]
                    else:
                        ...
                        cache[key] = (option_price, delta, price_se, delta_se)
                else:
                    option_price, delta, price_se, delta_se = price_and_delta_bermudan_put(
                        S0=S, K=K, r=r, q=q, sigma=sigma, T=T_rem, eps_rel=cfg.eps_rel, **cfg.engine_cfg
                    )

            # Early exercise heuristic (time value ~ 0)
            if cfg.exercise_detection and payoff > 0.0 and T_rem > 0.0:
                time_value = float(option_price - payoff)
                if time_value <= float(cfg.exercise_k_stderr) * float(max(price_se, 0.0)):
                    exercised_now = True

            # ---- Trading / accounting ----
            if i == 0:
                # receive premium first
            ...
            # if exercised, settle payoff and flatten hedge
            if exercised_now:
                ...
                # record the close-out trade as part of turnover/cost
                trade += trade2
                tcost += tcost2
